...
// Prepare dependencies. If require is available then it is used, otherwise
// a browser environment is assumed and dependencies are expected to be available
...
// is on. _This operation is asynchronious_.
LC2.prototype.run = function (cb) {
  var lc2 = this
  var toExec = () => lc2.step(step_cb)
  var step_cb = () => {
    if (this.isOn()) {
      ...
      // just looping synchroniously crashes the emulator.
      setTimeout(toExec, 0)
    }
    else if (cb) cb(!this.isOn())
  }
...
// If the module system is found, then export the constructor.
