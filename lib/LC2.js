...
LC2.prototype.step = function (done) {
  this.execute(this.memory[this.pc], done)
}

LC2.prototype.run = function (done) {
  var lc2 = this
  function exec (stop) {
    if (!stop) lc2.step(exec); else done()
  }
  exec()
}
...
LC2.prototype.br = function (nzp, dest) {
  if (this.debug) console.log('CC:', common.pad(this.cc.toString(2), 3), 'NZP:', common.pad(nzp.toString(2), 3))
  if ((this.cc & nzp) !== 0) {
...
LC2.prototype.str = function (dest, br, index6) {
  ...
  if (this.debug) console.log((this.gpr[dest]).toString(16))
  if (this.debug) console.log((this.gpr[br] + index6).toString(16))
  if (this.debug) console.log(this.memory[this.gpr[br] + index6])
}
...
LC2.prototype.trap = function (trapn, done) {
  ...
  if (trapn === 0x20 || trapn === 0x23) { // GETC, IN
    return this.readChar(trapn === 0x23, c => {
      if (this.debug) console.log('READ CHAR:', c)
      this.gpr[0] = c & 255
      done(false)
    })
    ...
    this.writeString(this.buildString(this.gpr[0]))
    ...
    console.log('[FATAL] Invalid subroutine address!')
  }
  done(false)
}
...
LC2.prototype.buildString = function (x) {
  ...
  while (this.memory[x] !== 0) {
    if (this.debug) console.log('[DEBUG] Building String:', this.memory[x], String.fromCharCode(this.memory[x]))
    str += String.fromCharCode(this.memory[x])
    x++
  }
