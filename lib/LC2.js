...
function LC2 (debug) {
  ...
  var c = new LC2Console(this, p, this.debug)
  c.loadSubroutines()
  this.peripherals = {}
  this.peripherals[p] = c
}
...
LC2.prototype.loadCode = function (code, dest) {
  ...
  if (this.debug) console.log('Loading binary:', code.length - i, 'words at', 'x' + common.pad(dest.toString(16), 4))
  for (i = 0; i < code.length - co; i++) {
...
LC2.prototype.mem = function (i, value) {
  if (value !== undefined) {
    ...
    if (this.peripherals && i >= this.peripherals_loc) {
      ...
      if (per !== undefined) {
        if (this.debug) console.log('Sending command to peripheral:', per.name)
        per.cmd(value)
      }
    }
  }
...
LC2.prototype.ldi = function (dest, src) {
  var addr = src | common.PC7msb(this.pc)
  this.gpr[dest] = this.mem(this.mem(addr))
