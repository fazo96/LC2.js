...
Assembler.prototype.parseInstruction = function (obj) {
  ...
  if (opname.indexOf('.') === 0) {
    ...
    console.log('[WARNING] Line ' + obj.row + ': Pseudoinstructions not implemented yet. Ignoring!')
    return { row: obj.row, opname: 'NOP', opcode: 0, args: [], string: x }
  } else {
    // This is an instruction
    var opcode = opname.indexOf('br') === 0 ? 0 : common.opnames.indexOf(opname.toLowerCase())
    if (opcode < 0) {
      console.log('[FATAL] INVALID INSTRUCTION', x)
    }
