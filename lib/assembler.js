...
Assembler.prototype.showErrors = function (arr) {
  return arr.reduce((a, b) => {
    if (Array.isArray(a)) {
      return a.concat(b.warnings || []).concat(b.errors || [])
    } else {
      let root = a.warnings || []
      return root.concat(a.errors || [], b.warnings || [], b.errors || [])
    }
  }, [])
}
...
Assembler.prototype.toList = function (arr) {
  ...
  return list.map(this.writeInstruction, this)
}
...
Assembler.prototype.writeInstruction = function (o, i) {
  ...
  var val2compl = common.toTwoComplement(val, 16)
  var hex = common.pad(val2compl.toString(16), 4).toUpperCase()
  var bin = common.pad(val2compl.toString(2), 16)
...
Assembler.prototype.toBinary = function (arr, switchEndian) {
  ...
  for (var i = 0; i < arr.length; i++) {
    if (typeof arr[i] === 'number') {
      ...
      arr[i].addError('Invalid instruction while postprocessing', this.debug)
    }
  }
...
Assembler.prototype.readInstruction = function (obj, i) {
  if (obj.isInstruction) return obj // Is an Instruction
  ...
  if (firstWord.indexOf('.') !== 0 && common.opnames[firstWord] === undefined && !/^br([nzp]+$)/.test(firstWord)) {
    ...
    if (words.length === 1) {
      // Has only label
      return new Instruction({ source: x, label: label, opname: 'NOP', args: [], row: obj.row, empty: true })
    }
  ...
  return new Instruction({ source: x, label: label, opname: opname, args: parts, row: obj.row })
}
...
Assembler.prototype.setLabel = function (label) {
  ...
  console.log('Setting Label:', label, 'to', this.symbolTable[label], 'whichs maps to', final)
  console.log('orig', this.orig)
}

function returnErrors (obj) {
  return { string: obj.string, errors: obj.errors || [], warnings: obj.warnings || [] }
}
...
Assembler.prototype.firstPass = function (x, i) {
  ...
  if (obj.row === this.rows && obj.opname !== '.END') {
    obj.addWarning('final instruction isn\'t .END', this.debug)
  }
  if (obj.label !== undefined) this.setLabel(obj.label.toLowerCase())
  if (i === 0 && obj.opname !== '.ORIG') {
    obj.addError('.ORIG has to be the first instruction!', this.debug)
    ...
    if (obj.opname === '.ORIG') {
      if (i !== 0) {
        ...
        obj.addError('.ORIG needs ONE argument, found ' + obj.args.length + ' instead!', this.debug)
        ret = obj
      } else {
        let val = this.readArgument(obj.args[0])
        if (val.literal === undefined) {
          obj.addError('.ORIG argument invalid', this.debug)
        } else {
          obj.value = val.literal
          this.orig = obj.value
        }
        ret = obj
      }
      ...
      if (obj.row !== this.rows) {
        obj.addWarning('.END has to be at the end of the file!', this.debug)
        ret = obj
      }
    } else if (obj.opname === '.FILL') {
      if (obj.args.length !== 1) {
        obj.addError('.FILL needs ONE argument, found ' + obj.args.length + ' instead!')
        ret = obj
      } else {
        ret = this.readArgument(obj.args[0])
        if (ret.literal === undefined) {
          obj.addError('.FILL argument invalid')
          ret = obj
        } else ret = ret.literal
      }
    } else if (obj.opname === '.BLKW') {
      if (obj.args.length < 1 || obj.args.length > 2) {
        obj.addError('.BLKW needs 1 or 2 arguments, found ' + obj.args.length + ' instead!', this.debug)
        ret = obj
      } else {
        ...
        console.log('Reserving', len, 'memory cells with content', content, 'for .BLKW call')
        this.currentInstrDelta += len - 1
      }
    } else if (obj.opname === '.CHAR' && this.magic) {
      if (obj.args.length !== 1) {
        obj.addError('.CHAR needs 1 argument, found ' + obj.args.length + ' instead!', this.debug)
        ret = obj
      } else {
      ...
      if (obj.args.length < 1) {
        obj.addError('.STRINGZ needs 1 argument, found ' + obj.args.length + ' instead!', this.debug)
        ret = obj
      } else {
        ...
        if (orig.charAt(0) === '"' && orig.charAt(orig.length - 1) === '"') {
          ...
          // Invalid String
          obj.addError('.STRINGZ argument needs to be wrapped in double quotes', this.debug)
          str = orig
        }
        ...
        console.log('Reserving', str.length + 1, 'memory cells for .STRINGZ call')
      }
    } else {
      obj.addError('Unimplemented or invalid pseudo instruction:' + obj.opname, this.debug)
      ret = obj
    }
  } else ret = x
  if (!obj.empty) this.currentInstrDelta++
  console.log('FINISHED FIRST PASS, currentInstrDelta', this.currentInstrDelta)
  return ret
}
...
Assembler.prototype.secondPass = function (x, i) {
  ...
  var obj = this.readInstruction(x)
  if (obj.hasErrors()) return returnErrors(obj)
  ...
  if (obj.opname === 'NOP') {
    ...
    obj.opcode = common.opnames[obj.opname.toLowerCase()]
  }
  if (obj.opcode < 0) {
    obj.addError('invalid instruction ' + x + ' opname:' + obj.opname, this.debug)
    return obj
  }
  obj.builtArgs = obj.args.map(this.readArgument, this)
...
Assembler.prototype.buildInstruction = function (x) {
  ...
  if (x.opcode === 0) {
    ...
    if (x.builtArgs[2].register !== undefined) {
      // Use register as third argument
      res = res | x.builtArgs[2].register | (src << 6) | (dst << 9)
    } else {
      // Use literal value as third argument
      res = res | common.toTwoComplement(x.builtArgs[2].literal, 5) | (src << 6) | (dst << 9) | 32
    }
...
Assembler.prototype.buildArgument = function (i, obj, skipTwoComplementConversion) {
  if (obj.args[i] === undefined) {
    obj.addError('missing required argument number ' + i, this.debug)
  } else {
    var x = obj.args[i]
