  ...
  if (i === 0 && obj.opname !== '.ORIG') {
    ...
    if (obj.opname === '.ORIG') {
      ...
      if (obj.args.length !== 2) {
        console.log('[FATAL] Line ' + obj.row + ': .BLKW needs TWO arguments, found ' + obj.args.length + ' instead!')
        ret = undefined
      } else {
        if (obj.label !== undefined) {
          this.symbolTable[obj.label] = i + this.orig - this.page - 1
        ...
        var content = this.parseArgument(obj.args[1]).literal
        if (!content || !len) {
          console.log('[FATAL] Line ' + obj.row + ': invalid .BLKW arguments')
          ret = undefined
        } else ret = new Array(len).fill(content)
      }
    } else {
